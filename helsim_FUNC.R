...
readParams <- function (fileName,demogName="Default") 
{
	...
	skipYear <- as.numeric(readParam("skipYear",fileName))
	#stopTreat <- treatmentStart + treatInterval * (nRounds - 1) + as.numeric(skipYear > 0)
	stopTreat <- maxTime
	chemoTimings <- c()
	if(skipYear > 1 & skipYear < stopTreat)
	{
		chemoTimings <- c(seq(from=treatmentStart, to=(skipYear - treatInterval), by=treatInterval), seq(from=skipYear+1, to=stopTreat, by=treatInterval))
	}else if(skipYear == 1)
	{
		chemoTimings <- seq(from=treatmentStart+1, to=stopTreat+1, by=treatInterval)
	}else if(skipYear==stopTreat)
	{
		chemoTimings <- seq(from=treatmentStart, to=stopTreat-1, by=treatInterval)
	}else
	{
