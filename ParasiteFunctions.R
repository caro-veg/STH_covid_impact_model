...
# 
# To do: 
# 1 - replace the expression for dL expression with one equivalent to the dM expression. 
# 2 - replace integrals with trapezium versions. 
# Author: jtrusc
###############################################################################
...
## output from the multi-chain batch job Schisto_ageProfile.R contains 
## a list of metrop outputs. This turns the chains into one long one. 
aggregateMCMCChains <- function(MCMCOuput)
{
	outChain <- MCMCOuput$MCMC_Chains[[1]]$batch
	for(i in 2:length(MCMCOuput$MCMC_Chains))
	{
		outChain <- rbind(outChain,MCMCOuput$MCMC_Chains[[i]]$batch)
	}
	return(outChain)
}

## a function to find the MLE and CI interval from the columns of an MCMC chain in which the last column is LL. 
## CI interval is in percent. 
MaxLLAndCI <- function(theChain,CI)
{
	dims <- dim(theChain)
	lower <- round(dims[1]*(100-CI)/200)
	upper <- dims[1] - lower
	indexML <- order(theChain[,dims[2]],decreasing=TRUE)[1]
	ans <- matrix(ncol=(dims[2]-1),nrow=3)
	for(i in 1:(dims[2]-1))
	{
		currentOrder <- sort(theChain[,i])
		ans[,i] <- c(currentOrder[lower],theChain[indexML,i],currentOrder[upper])
	}
	colnames(ans) <- colnames(theChain)[-dims[2]]
	rownames(ans) <- c(paste(as.character((100-CI)/2),"%",sep=""),"MLE",paste(as.character(50+CI/2),"%",sep=""))
	return(list(CIandMLE=ans,ML=theChain[indexML,dims[2]]))
}
...
## New calculation of R0 with higher resolution too. 
getPsiParasites <- function(pCurrent)
{
	## higher resolution. 
	deltaT <- 0.1
	...
	#psi <- pCurrent$R0*MeanLifespan*(meanBeta + pCurrent$lDecayRate)/(pCurrent$lambda*pCurrent$z*summation) ## Only applicable when contat process consumes material. 
...
#### Assign parameters to list structure. 
#readDetParams <- function(paramFilePath,demogName="Default")
#{
#	params <- list()
